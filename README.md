...
```
nba_api  →  etl_pipeline.py  →  SQLite DB (player_careers, ~500+ active players)
                                    │
...
```bash
python etl_pipeline.py        # pulls all active players from nba_api into SQLite (long-running)
...
- **Elo ratings are still scoped to 3 players.** `player_careers` now covers the full active roster, but `build_elo_ratings.py` still hardcodes `PLAYER_NAMES = ["LeBron James", "Russell Westbrook", "Stephen Curry"]`. Expanding this to the full roster is the next step — it requires a full-roster self-join on game logs, which is a heavier computation than the current 3-player version.
- **The frontend UI hasn't caught up to the roster expansion.** `static/index.html` still only shows LeBron, Curry, and Westbrook as sidebar chips, and the welcome message still names only those three. The backend can already answer questions about any active player via free-text input; the sidebar just hasn't been regenerated yet.
...
├── nba_database.db              # SQLite database (generated by ETL + Elo scripts)
